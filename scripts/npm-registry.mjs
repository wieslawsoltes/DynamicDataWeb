...
async function verify(tarball, distTag) {
  ...
  try {
    ...
    await mkdir(fresh);
    await writeFile(join(fresh, 'package.json'), JSON.stringify({ name: 'dynamicdataweb-public-registry-consumer', version: '1.0.0', private: true, type: 'module' }));
    ...
    const smoke = spawnSync(process.execPath, ['--input-type=module', '-e', `import assert from 'node:assert/strict'; import { SourceCache } from ${JSON.stringify(pkg.name)}; import { Observable } from 'rxjs'; const source = new SourceCache(item => item.id); assert(source.connect() instanceof Observable); source.addOrUpdate({id:1}); assert.equal(source.count, 1); source.dispose();`], { cwd: fresh, stdio: 'inherit', timeout: 30_000 });
    ...
    await summary(`Published and verified **${pkg.name}@${pkg.version}** on npm (${distTag}).\n\nRelease and registry tarballs have identical SHA512 integrity. Public download, provenance metadata, ESM/CommonJS/TypeScript consumers, subpath identity, external RxJS interoperability, and fresh npm installation checks passed.\n\n[View npm package](https://www.npmjs.com/package/${pkg.name}/v/${pkg.version})`);
  } finally {
