...
__export(index_exports, {
  ...
  AsArray: () => asArray,
  AsList: () => asList,
  AsObservableCache: () => asObservableCache,
  AsObservableList: () => asObservableList,
  AsWatcher: () => asWatcher,
  AsyncDisposeMany: () => asyncDisposeMany,
  AutoRefresh: () => autoRefresh,
  AutoRefreshOnObservable: () => autoRefreshOnObservable,
  Average: () => average,
  Avg: () => avg,
  Batch: () => batch,
  BatchIf: () => batchIf,
  ...
  Bind: () => bind,
  BindToObservableCollection: () => bindToObservableCollection,
  BindToObservableList: () => bindToObservableList,
  BindingOptions: () => BindingOptions,
  BufferIf: () => bufferIf,
  BufferInitial: () => bufferInitial,
  Cast: () => cast,
  CastToObject: () => castToObject,
  Change: () => Change,
  ChangeAwareCache: () => ChangeAwareCache,
  ChangeAwareList: () => ChangeAwareList,
  ChangeKey: () => changeKey,
  ...
  ConvertOr: () => convertOr,
  Count: () => count,
  CreateObservableObject: () => createObservableObject,
  ...
  Filter: () => filter,
  FilterImmutable: () => filterImmutable,
  FilterOnObservable: () => filterOnObservable,
  FilterOnProperty: () => filterOnProperty,
  FilterWithState: () => filterWithState,
  FinallySafe: () => finallySafe,
  ...
  ObservableChangeSet: () => ObservableChangeSet,
  ...
  Sort: () => sort,
  SortAndBind: () => sortAndBind,
  SortAndBindOptions: () => SortAndBindOptions,
  SortAndPage: () => sortAndPage,
  SortAndVirtualise: () => sortAndVirtualise,
  SortAndVirtualize: () => sortAndVirtualize,
  SortBy: () => sortBy,
  ...
  TransformImmutable: () => transformImmutable,
  TransformMany: () => transformMany,
  TransformManyAsync: () => transformManyAsync,
  TransformManySafeAsync: () => transformManySafeAsync,
  TransformOnObservable: () => transformOnObservable,
  TransformSafe: () => transformSafe,
  TransformSafeAsync: () => transformSafeAsync,
  TransformToTree: () => transformToTree,
  TransformWithInlineUpdate: () => transformWithInlineUpdate,
  TreatMovesAsRemoveAdd: () => treatMovesAsRemoveAdd,
  TrueForAll: () => trueForAll,
  TrueForAny: () => trueForAny,
  ...
  Watcher: () => Watcher,
  WhenAnyPropertyChanged: () => whenAnyPropertyChanged,
  WhenChanged: () => whenChanged,
  WhenPropertyChanged: () => whenPropertyChanged,
  ...
  scheduleRecurringAction: () => scheduleRecurringAction,
  selectValues: () => selectValues,
  setObservableDecorator: () => setObservableDecorator,
  skipInitial: () => skipInitial,
...
function disposable(action) {
  const sub = new import_rxjs.Subscription(action);
...
var SourceBase = class {
  constructor(kind) {
    ...
    this._previews = new import_rxjs.Subject();
    this._counts = new import_rxjs.Subject();
    ...
    this._version = 0;
    this._lastCount = 0;
    this.countChanged = observableDecorator(new import_rxjs.Observable((observer) => {
      if (this._disposed) {
        observer.complete();
        return;
      }
  ...
  _observe(preview, predicate, suppressEmpty) {
    ...
    return observableDecorator(new import_rxjs.Observable((observer) => {
      if (this._disposed) {
        observer.complete();
        return;
      }
...
var SourceCache = class extends SourceBase {
  ...
  watch(key) {
    return observableDecorator(new import_rxjs.Observable((observer) => this.connect((_, candidate) => Object.is(candidate, key)).subscribe({ next: (changes) => {
...
var observable = (value) => (0, import_rxjs2.isObservable)(value) || !!value?.subscribe;
...
function derived(setup) {
  return (source) => new import_rxjs2.Observable((observer) => {
    const subscriptions = new import_rxjs2.Subscription(), model = new Model();
...
function tracker(hooks = {}) {
  ...
  const add = (item, key, index = state.entries.length, previous, prior) => {
    ...
    const entry = { item, key, prior, active: true, subscription: new import_rxjs3.Subscription() };
    if (state.kind === "cache") {
...
function propertyEvents(item) {
  ...
  if (!subject) notifications.set(target, subject = new import_rxjs3.Subject());
  return subject;
}
...
function changedProperty(item, property, notifyInitial = true) {
  if (typeof property === "string" && property.includes(".")) {
    ...
    return new import_rxjs3.Observable((observer) => {
      let connections = new import_rxjs3.Subscription(), currentValue = readProperty(item, property);
      const rewire = () => {
        connections.unsubscribe();
        connections = new import_rxjs3.Subscription();
        let current = item;
...
function whenPropertyChanged(itemOrChanges, property, notifyInitial = true) {
  return (0, import_rxjs3.isObservable)(itemOrChanges) ? itemOrChanges.pipe(mergeMany((item) => changedProperty(item, property, notifyInitial))) : changedProperty(itemOrChanges, property, notifyInitial);
}
function observeProperty(itemOrChanges, property, notifyInitial = true) {
  return whenPropertyChanged(itemOrChanges, property, notifyInitial).pipe((0, import_rxjs3.map)((event) => event.value), (0, import_rxjs3.distinctUntilChanged)());
}
...
function autoRefreshOnObservable(selector, options = {}) {
  ...
  return (source) => new import_rxjs3.Observable((observer) => {
    ...
    const resources = new import_rxjs3.Subscription();
    let timer2;
    const refresh2 = (entry) => {
      ...
      if (options.buffer > 0) {
        if (!timer2) {
          timer2 = (options.scheduler ?? import_rxjs3.asyncScheduler).schedule(() => {
            timer2 = null;
            flush();
          }, options.buffer);
    ...
    state = tracker({ add(entry) {
      entry.subscription.add((0, import_rxjs3.from)(selector(entry.item, entry.key)).subscribe({ next: () => refresh2(entry), error: (error) => observer.error(error) }));
    } });
...
function autoRefresh(property, options = {}) {
  ...
  return autoRefreshOnObservable((item) => {
    ...
    return options.throttle > 0 ? events.pipe((0, import_rxjs3.debounceTime)(options.throttle, options.scheduler ?? import_rxjs3.asyncScheduler)) : events;
  }, options);
}
  ...
  return (source) => new import_rxjs3.Observable((observer) => {
    ...
    const start = (entry, previous) => {
      ...
      try {
        ...
        observable2 = (0, import_rxjs3.isObservable)(selected) || selected?.then ? (0, import_rxjs3.from)(selected) : (0, import_rxjs3.of)(selected);
      } catch (error) {
        if (options.onError) {
    ...
    state = tracker({
      ...
      refresh(entry) {
        if (options.transformOnRefresh) {
          entry.subscription.unsubscribe();
          entry.subscription = new import_rxjs3.Subscription();
...
function transformAsync(factory, options = {}) {
  ...
  return (source) => new import_rxjs3.Observable((observer) => {
    ...
    const queued = (...arguments_) => new import_rxjs3.Observable((destination) => {
      ...
      const job = { closed: false, start() {
        ...
        try {
          ...
          inner = ((0, import_rxjs3.isObservable)(value) || value?.then ? (0, import_rxjs3.from)(value) : (0, import_rxjs3.of)(value)).subscribe({ next: (value2) => destination.next(value2), error(error) {
            finish();
...
function mergeMany(selector) {
  return (source) => new import_rxjs3.Observable((observer) => {
    ...
    const state = tracker({ add(entry) {
      entry.subscription.add((0, import_rxjs3.from)(selector(entry.item, entry.key)).subscribe({ next: (value) => observer.next(value), error: (error) => observer.error(error) }));
    } });
    resources.add(() => state.dispose());
    resources.add(source.subscribe({ next(changes) {
      try {
...
function mergeManyItems(selector) {
  return mergeMany((item, key) => (0, import_rxjs3.from)(selector(item, key)).pipe((0, import_rxjs3.map)((value) => ({ item, key, value }))));
}
...
function onItem(reason2, action) {
  return (source) => new import_rxjs3.Observable((observer) => source.subscribe({ next(changes) {
    try {
      for (const change of getChanges(changes)) {
...
function notEmpty() {
  return (0, import_rxjs3.filter)((changes) => getChanges(changes).length > 0);
}
function skipInitial() {
  return (0, import_rxjs3.skip)(1);
}
...
function watch(key) {
  return (source) => source.pipe((0, import_rxjs3.map)((changes) => [...getChanges(changes)].filter((change) => Object.is(change.key, key))), (0, import_rxjs3.filter)((changes) => changes.length > 0), mergeChangeRecords());
}
...
function batch(duration = 16, scheduler = import_rxjs3.asyncScheduler) {
  if (!(duration > 0)) throw new RangeError("Batch duration must be positive");
  return (source) => source.pipe((0, import_rxjs3.bufferTime)(duration, scheduler), (0, import_rxjs3.filter)((sets) => sets.some((changes) => getChanges(changes).length)), (0, import_rxjs3.map)((sets) => cs(sets.flatMap((changes) => [...getChanges(changes)]), sets[0]?.kind)));
}
  ...
  return (source) => new import_rxjs3.Observable((observer) => {
    ...
    const armTimeout = () => {
      if (paused && options.timeout != null && !timeout) {
        timeout = (options.scheduler ?? import_rxjs3.asyncScheduler).schedule(() => {
          paused = false;
          flush();
        }, options.timeout);
...
function expireAfter(selector, options = {}, sourceOptions = {}) {
  ...
  const scheduler = options.scheduler ?? import_rxjs3.asyncScheduler;
  return (source) => new import_rxjs3.Observable((observer) => {
    const resources = new import_rxjs3.Subscription();
...
function disposalOperator(disposer, completedAccessor) {
  return (source) => new import_rxjs3.Observable((observer) => {
    const completion = new import_rxjs3.ReplaySubject(1);
    const resources = new import_rxjs3.Subscription();
...
function bufferInitial(duration, scheduler = import_rxjs3.asyncScheduler) {
...
var finallySafe = (action) => (0, import_rxjs3.finalize)(action);
...
var watchValue = (key) => (source) => (typeof source.connect === "function" ? source.connect() : source).pipe(watch(key), (0, import_rxjs3.map)((change) => change.current));
function whenAnyPropertyChanged(itemOrChanges, ...propertyNames) {
  const observe = (item) => propertyEvents(item).pipe((0, import_rxjs3.filter)((event) => !propertyNames.length || propertyNames.includes(event.propertyName)), (0, import_rxjs3.map)(() => item));
  return (0, import_rxjs3.isObservable)(itemOrChanges) ? itemOrChanges.pipe(mergeMany(observe)) : observe(itemOrChanges);
}
function whenChanged(item, properties, resultSelector) {
  const fields = Array.isArray(properties) ? properties : [properties];
  return (0, import_rxjs3.combineLatest)(fields.map((property) => observeProperty(item, property))).pipe((0, import_rxjs3.map)((values) => resultSelector ? resultSelector(item, ...values) : values.length === 1 ? values[0] : values));
}
function observeCollectionChanges(collection, eventName = "collectionchange") {
  const events = typeof collection.connect === "function" ? collection.connect() : (0, import_rxjs3.isObservable)(collection) ? collection : (0, import_rxjs3.fromEvent)(collection, eventName);
  return events.pipe((0, import_rxjs3.map)((event) => ({ sender: collection, eventArgs: event?.detail ?? event })));
}
function trueFor(selector, condition, all) {
  return (source) => new import_rxjs3.Observable((observer) => {
    ...
    state = tracker({ add(entry) {
      entry.subscription.add((0, import_rxjs3.from)(selector(entry.item, entry.key)).subscribe({ next(value) {
        entry.value = value;
        entry.hasValue = true;
        evaluate();
      }, error: (error) => observer.error(error) }));
...
function refCount() {
  return (source) => {
    let session;
    return new import_rxjs3.Observable((observer) => {
      if (!session) session = { subject: new import_rxjs3.Subject(), references: 0, state: void 0, kind: "cache", upstream: new import_rxjs3.Subscription() };
...
function switchLatest() {
  return (sources) => new import_rxjs3.Observable((observer) => {
    const resources = new import_rxjs3.Subscription();
    ...
    resources.add(sources.subscribe({ next(source) {
      ...
      inner = (0, import_rxjs3.from)(incoming).subscribe({ next(changes) {
...
var Group = class {
  constructor(key, kind = "cache") {
    ...
    this.kind = kind;
    this._data = kind === "cache" ? /* @__PURE__ */ new Map() : [];
    this._subject = new import_rxjs4.Subject();
...
var ImmutableGroup = class {
  ...
  connect() {
    return (0, import_rxjs4.of)(this.kind === "cache" ? set(this._entries.map(([key, current]) => ({ reason: "add", key, current }))) : set(this.size ? [{ reason: "addRange", range: { items: this.items.slice(), index: 0 } }] : [], "list"));
  }
  Connect() {
    return this.connect();
  }
...
function grouping(selector, regrouper, immutable = false) {
  if (typeof selector !== "function" && !(0, import_rxjs4.isObservable)(selector)) throw new TypeError("A group selector function or observable is required");
  return (source) => new import_rxjs4.Observable((observer) => {
    ...
    if ((0, import_rxjs4.isObservable)(selector)) subscriptions.add(selector.subscribe({ next: (next) => {
...
function groupOnObservable(selector) {
  return (source) => new import_rxjs4.Observable((observer) => {
    const subs = new import_rxjs4.Subscription(), forwarded = new import_rxjs4.Subject(), regroup = new import_rxjs4.Subject(), streams = /* @__PURE__ */ new Map(), values = /* @__PURE__ */ new Map();
      ...
      try {
        for (const c of changes) {
          ...
          if (r === "add" || r === "update") {
            ...
            if (!(0, import_rxjs4.isObservable)(stream)) throw new TypeError("groupOnObservable selector must return an Observable");
            active.add(c.key);
            const sub = stream.pipe((0, import_rxjs4.distinctUntilChanged)()).subscribe({ next: (value) => {
...
function join(type, rightSource, foreignKeySelector, resultSelector, many2 = false) {
  ...
  return (leftSource) => new import_rxjs4.Observable((observer) => {
    const subs = new import_rxjs4.Subscription(), left = /* @__PURE__ */ new Map(), right = /* @__PURE__ */ new Map(), rightForeign = /* @__PURE__ */ new Map(), buckets = /* @__PURE__ */ new Map();
...
function combine(sources, operator = "or") {
  ...
  return new import_rxjs4.Observable((observer) => {
    const subs = new import_rxjs4.Subscription(), entries = [], output = /* @__PURE__ */ new Map(), pendingLatest = /* @__PURE__ */ new Map();
...
var invalidateWhen = (invalidate) => (source) => asStream(invalidate).pipe((0, import_rxjs4.startWith)(void 0), (0, import_rxjs4.switchMap)(() => source), (0, import_rxjs4.distinctUntilChanged)());
function groupWithSpecifiedGroups(selector, resultGroupSource) {
  return (source) => new import_rxjs4.Observable((observer) => {
    const subs = new import_rxjs4.Subscription(), available = /* @__PURE__ */ new Map(), exposed = /* @__PURE__ */ new Map(), inner = /* @__PURE__ */ new Map();
...
var filteredChanges = (predicate) => (source) => source.pipe((0, import_rxjs5.map)((changes) => projectedSet(changes, predicate)), (0, import_rxjs5.filter)((changes) => changes.length > 0));
...
function flattenBufferResult() {
  return (source) => source.pipe((0, import_rxjs5.filter)((buffer) => buffer.length > 0), (0, import_rxjs5.map)((buffer) => {
...
function treatMovesAsRemoveAdd() {
  return (0, import_rxjs5.map)((changes) => {
...
function forEachChange(action) {
  ...
  return (0, import_rxjs5.tap)((changes) => {
...
function flattenChanges() {
  return (0, import_rxjs5.mergeMap)((changes) => Array.from(changes));
}
var flatten = flattenChanges;
function startWithEmpty(kind = "cache") {
  return (0, import_rxjs5.startWith)(new ChangeSet([], kind));
}
function startWithItem(item, key = item?.key ?? item?.Key ?? item?.id) {
  return (0, import_rxjs5.startWith)(new ChangeSet([{ reason: "add", key, current: item }], "cache"));
}
function removeIndex() {
  return (0, import_rxjs5.map)((changes) => new ChangeSet(withoutIndex(changes), kindOf(changes)));
}
...
function ensureUniqueKeys() {
  return (source) => (0, import_rxjs5.defer)(() => {
    const state = /* @__PURE__ */ new Map();
    return source.pipe((0, import_rxjs5.map)((changes) => {
...
function clone(target) {
  return (0, import_rxjs5.tap)((changes) => copyInto(target, changes));
}
...
function populateFrom(destination, observable2, observer) {
  ...
  return observable2.pipe((0, import_rxjs5.tap)((items) => destination.addOrUpdate(items))).subscribe(observer);
}
function toObservableOptional(key, initialOptionalWhenMissing = false, comparer = Object.is) {
  ...
  return (source) => new import_rxjs5.Observable((observer) => {
...
function adapt(adapter) {
  ...
  return (0, import_rxjs5.tap)(action);
}
...
function updateIndex(setter = (item, index) => {
  ...
  return (source) => (0, import_rxjs5.defer)(() => {
    let state;
    return source.pipe((0, import_rxjs5.tap)((changes) => {
...
function collectUpdateStats() {
  return (source) => (0, import_rxjs5.defer)(() => {
    ...
    return source.pipe((0, import_rxjs5.map)((input) => {
...
function transformToTree(pivotOn, predicateChanged) {
  ...
  return (source) => new import_rxjs5.Observable((observer) => {
    ...
    const subscriptions = new import_rxjs5.Subscription();
    const pending = [];
...
function mergedStreams(outer, selector, options = {}) {
  ...
  return new import_rxjs5.Observable((observer) => {
    const subscriptions = new import_rxjs5.Subscription(), records = [], parents = /* @__PURE__ */ new Map(), parentList = [];
    ...
    function add(stream, item, key) {
      const record = { stream, item, key, state: null, order: /* @__PURE__ */ new Map(), alive: true, done: false, subscription: new import_rxjs5.Subscription() };
      records.push(record);
...
function mergeChangeSets(sourcesOrOptions, options = {}) {
  if (Array.isArray(sourcesOrOptions)) return mergedStreams((0, import_rxjs5.from)(sourcesOrOptions), null, options);
  if (isStream(sourcesOrOptions)) return (source) => mergedStreams((0, import_rxjs5.from)([source, sourcesOrOptions]), null, options);
...
var ChangeSetAggregator = class {
  constructor(source, options = {}) {
    ...
    const updates = new import_rxjs6.Subject();
    this._subscriptions = new import_rxjs6.Subscription();
...
var Watcher = class {
  ...
  watch(key) {
    ...
    return this.scheduler ? observable2.pipe((0, import_rxjs6.observeOn)(this.scheduler)) : observable2;
  }
  Watch(key) {
...
function retryWithBackOff(strategyOrOptions = {}) {
  ...
  const scheduler = options.scheduler ?? import_rxjs6.asyncScheduler;
  ...
  return (0, import_rxjs6.retry)({
    ...
    delay(error, retryCount) {
      ...
      return (0, import_rxjs6.timer)(delay, scheduler);
    }
  });
}
function scheduleRecurringAction(scheduler, interval, action) {
  if (typeof scheduler?.schedule !== "function") {
    action = interval;
    interval = scheduler;
    scheduler = import_rxjs6.asyncScheduler;
  }
  ...
  const subscription = new import_rxjs6.Subscription();
...
var optionalMap = (source, project) => (0, import_rxjs7.isObservable)(source) ? source.pipe((0, import_rxjs7.map)((value) => project(requireOptional(value)))) : project(requireOptional(source));
...
function toOptional(source) {
  if (!arguments.length) return (input) => (0, import_rxjs7.isObservable)(input) ? input.pipe((0, import_rxjs7.map)((value) => Optional.of(value))) : Optional.of(input);
  return (0, import_rxjs7.isObservable)(source) ? source.pipe((0, import_rxjs7.map)((value) => Optional.of(value))) : Optional.of(source);
}
...
function valueOr(sourceOrFallback, maybeFallback) {
  ...
  if (direct && !isOptional(sourceOrFallback) && !(0, import_rxjs7.isObservable)(sourceOrFallback)) return sourceOrFallback ?? fallbackValue(fallback);
  return direct ? optionalMap(sourceOrFallback, project) : (source) => optionalMap(source, project);
}
...
function valueOrThrow(sourceOrException, maybeException) {
  const direct = isOptional(sourceOrException) || (0, import_rxjs7.isObservable)(sourceOrException);
...
function optionalEffect(onPresent, sourceOrAction, actionOrElse, maybeElse) {
  const direct = isOptional(sourceOrAction) || (0, import_rxjs7.isObservable)(sourceOrAction);
  ...
  const run = (source) => {
    if ((0, import_rxjs7.isObservable)(source)) return source.pipe((0, import_rxjs7.tap)(effect));
    effect(source);
    return source;
  };
...
function selectValues(source) {
  const run = (input) => {
    if ((0, import_rxjs7.isObservable)(input)) return input.pipe((0, import_rxjs7.filter)((value) => requireOptional(value).hasValue), (0, import_rxjs7.map)((value) => value.value));
    const values = [];
...
var DynamicDataOptions = Object.freeze({ binding: new BindingOptions(), scheduler: import_rxjs8.asyncScheduler });
...
var minimum = min;
var maximum = max;
var isEmpty = () => (source) => source.pipe(count(), (0, import_rxjs8.map)((n) => n === 0), (0, import_rxjs8.distinctUntilChanged)());
var isNotEmpty = () => (source) => source.pipe(count(), (0, import_rxjs8.map)((n) => n !== 0), (0, import_rxjs8.distinctUntilChanged)());
...
var observeOn2 = (scheduler) => (0, import_rxjs8.observeOn)(scheduler);
var subscribeOn = (scheduler) => (0, import_rxjs8.subscribeOn)(scheduler);
var observeOnDispatcher = () => (0, import_rxjs8.observeOn)(typeof requestAnimationFrame === "function" ? import_rxjs8.animationFrameScheduler : import_rxjs8.asyncScheduler);
var decorated = /* @__PURE__ */ new WeakSet();
var operatorRegistry = {};
function fluent(observable2) {
  if (!(observable2 instanceof import_rxjs8.Observable) && !observable2?.subscribe) return observable2;
